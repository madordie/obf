...
- (id)initWith(capacity):(size_t)numBytes;
...
- (id)initWith(data):(NSMutableData *)d
...
- (id)initWith(data):(NSData *)d timeout:(NSTimeInterval)t tag:(long)i;
@end
...
- (id)initWith(tLSSettings):(NSDictionary *)settings;
@end
...
- (id)init
{
	return [self initWith(delegate):nil delegateQueue:NULL socketQueue:NULL];
}

- (id)initWith(socketQueue):(dispatch_queue_t)sq
{
	return [self initWith(delegate):nil delegateQueue:NULL socketQueue:sq];
}

- (id)initWith(delegate):(id)aDelegate delegateQueue:(dispatch_queue_t)dq
{
	return [self initWith(delegate):aDelegate delegateQueue:dq socketQueue:NULL];
}

- (id)initWith(delegate):(id<GCDAsyncSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	if((self = [super init]))
	{
		delegate = aDelegate;
		...
		preBuffer = [[GCDAsyncSocketPreBuffer alloc] initWith(capacity):(1024 * 4)];
...
+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<GCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq error:(NSError* __autoreleasing *)error
{
  ...
  GCDAsyncSocket *socket = [[[self class] alloc] initWith(delegate):aDelegate delegateQueue:dq socketQueue:sq];
...
- (void)set(delegate):(id)newDelegate synchronously:(BOOL)synchronously
{
...
- (void)set(delegate):(id)newDelegate
{
	[self set(delegate):newDelegate synchronously:NO];
}

- (void)synchronouslySetDelegate:(id)newDelegate
{
	[self set(delegate):newDelegate synchronously:YES];
}
...
- (void)set(delegateQueue):(dispatch_queue_t)newDelegateQueue synchronously:(BOOL)synchronously
{
	dispatch_block_t block = ^{
...
- (void)set(delegateQueue):(dispatch_queue_t)newDelegateQueue
{
	[self set(delegateQueue):newDelegateQueue synchronously:NO];
}

- (void)synchronouslySetDelegateQueue:(dispatch_queue_t)newDelegateQueue
{
	[self set(delegateQueue):newDelegateQueue synchronously:YES];
}
...
- (void)set(delegate):(id)newDelegate delegateQueue:(dispatch_queue_t)newDelegateQueue synchronously:(BOOL)synchronously
{
	dispatch_block_t block = ^{
...
- (void)set(delegate):(id)newDelegate delegateQueue:(dispatch_queue_t)newDelegateQueue
{
	[self set(delegate):newDelegate delegateQueue:newDelegateQueue synchronously:NO];
}

- (void)synchronouslySetDelegate:(id)newDelegate delegateQueue:(dispatch_queue_t)newDelegateQueue
{
	[self set(delegate):newDelegate delegateQueue:newDelegateQueue synchronously:YES];
}
...
- (void)set(IPv4Enabled):(BOOL)flag
{
	// Note: YES means kIPv4Disabled is OFF
...
- (void)set(IPv6Enabled):(BOOL)flag
{
	// Note: YES means kIPv6Disabled is OFF
...
- (void)set(IPv4PreferredOverIPv6):(BOOL)flag
{
	// Note: YES means kPreferIPv6 is OFF
...
- (void) set(alternateAddressDelay):(NSTimeInterval)delay {
    dispatch_block_t block = ^{
...
- (void)set(userData):(id)arbitraryUserData
{
	dispatch_block_t block = ^{
...
- (BOOL)doAccept:(int)parentSocketFD
{
	...
	if (delegateQueue)
	{
		...
		dispatch_async(delegateQueue, ^{ @autoreleasepool {
			...
			GCDAsyncSocket *acceptedSocket = [[[self class] alloc] initWith(delegate):theDelegate
                        ...
                        tag:(long)tag
{
	...
	GCDAsyncReadPacket *packet = [[GCDAsyncReadPacket alloc] initWith(data):buffer
	                                                          startOffset:offset
	                                                            maxLength:length
	                                                              timeout:timeout
	                                                           readLength:0
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	GCDAsyncWritePacket *packet = [[GCDAsyncWritePacket alloc] initWith(data):data timeout:timeout tag:tag];
...
- (void)startTLS:(NSDictionary *)tlsSettings
{
	...
	GCDAsyncSpecialPacket *packet = [[GCDAsyncSpecialPacket alloc] initWith(tLSSettings):tlsSettings];
...
- (void)ssl_startTLS
{
	...
	sslPreBuffer = [[GCDAsyncSocketPreBuffer alloc] initWith(capacity):(1024 * 4)];
 ...
 * See header file for big discussion of this method.
**/
- (void)set(autoDisconnectOnClosedReadStream):(BOOL)flag
{
	// Note: YES means kAllowHalfDuplexConnection is OFF
